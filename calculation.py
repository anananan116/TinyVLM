...
class calculate_metrics:
    ...
    def process_result(self, results_dir):
        ...
        all_gts = []  # Track all ground truths
        all_preds = []  # Track all predictions
        total_score = 0
        for eval_type, task_name_list in eval_type_dict.items():
            ...
            for task_name in task_name_list:
                ...
                lines = open(task_txt, 'r', encoding='utf8').readlines()
                ...
                task_score_dict[task_name] = (task_score, task_acc)
                scores += task_score
            total_score += scores
            print("total score:", scores, "\n")
            for task_name, (score, acc) in task_score_dict.items():
                print("\t", task_name, " score:", score, "\t", f" acc:{acc:.2f}%")
            print("\n")
        ...
        print(f"Overall Accuracy: {overall_metrics['acc']*100:.2f}%")
        print(f"Overall Precision: {overall_metrics['precision']*100:.2f}%")
        print(f"Overall Recall: {overall_metrics['recall']*100:.2f}%")
        print(f"Total Other Answers: {overall_metrics['other_num']}")
        print(f"Confusion Matrix:")
        print(f"TP: {overall_metrics['TP']}, FN: {overall_metrics['FN']}")
        print(f"FP: {overall_metrics['FP']}, TN: {overall_metrics['TN']}")
        print("\n")
...
if __name__ == "__main__":
    ...
    for answer in answers:
        category, img_id = answer[0].split('/')
        with open(f"Formatted_Results/{category}.txt", 'a', encoding='utf8') as f:
            f.write(f"{img_id.strip()}\t{answer[1].strip()}\t{answer[2].strip()}\t{answer[3].replace('\n', '').strip()}\n")
